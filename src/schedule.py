...
dictionary, professors = [], []


def do_text(dictionary):
    all = []
...
def find_group(text):
    global dictionary
    return dictionary[int((datetime.datetime.now()).strftime('%Y')) % 100
                      - int(text.split('-')[-1])][text]


def find_professor(string):
    global professors
    ar = []
...
def get_schedule(first=False):
    global professors, dictionary
    if os.path.exists('data/data_file.json'):
        with open('data/data_file.json', 'r', encoding='utf-8') as f:  # открыли файл с данными, спс за инфу)
            dictionary = json.load(f)

    if os.path.exists('data/data_file2.json'):
        with open('data/data_file2.json', 'r', encoding='utf-8') as f:  # открыли файл с данными, спс за инфу)
            professors = json.load(f)

    if not os.path.exists('data/data_file.json') \
            or not os.path.exists('data/data_file2.json')\
            or datetime.datetime.now().strftime('%H') != dictionary['time']\
            or first:
                    ...
                    for week in range(6):
                        ...
                        for subject in range(6):
                            if clean(sheet.cell(week * 12 + subject * 2 + 3 + chet, group + 2).value):
                                ...
                                for pre in array:
                                    if pre not in professors.keys() and pre not in ' \n':
                                        professors[pre] = par()

                                for pre in array:
                                    if pre not in ' \n':
                                        professors[pre][str(chet)][str(week)][str(subject)] \
                            ...
                            weekdict[str(subject)] = str(sheet.cell(week * 12 + subject * 2 + 3 + chet, group).value) \
                                + ' ,  ' \
                                + str(sheet.cell(week * 12 + subject * 2 + 3 + chet, group + 1).value) + ' ,  ' \
                                + str(sheet.cell(week * 12 + subject * 2 + 3 + chet, group + 2).value) + ' ,  ' \
                                + str(sheet.cell(week * 12 + subject * 2 + 3 + chet, group + 3).value)

                        chetdict[str(week)] = weekdict
                    group_dict[str(chet)] = chetdict
                course_number[sheet.cell(1, group).value] = group_dict
            dictionary[i] = course_number
        dictionary['time'] = datetime.datetime.now().strftime('%H')
        ...
        with open('data/data_file2.json', 'w') as write_file:
            json.dump(professors, write_file)
